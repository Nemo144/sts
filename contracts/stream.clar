...
;;creating a public function for the stream 
(define-public (stream-to 
    ...
    (let (
        ...
        (current-stream-id (var-get latest-stream-id));;creating a temporary variable
     ) 
     ;;transfer stx to my contract address
...
;;refuel function for the creator of the stream to add additional tokens later
...
(define-public (withdraw
  ...
  (let (
    ...
    (try! (stx-transfer? balance tx-sender (get recipient stream)))
  )
)
...
;;update details function for the signature assent
(define-public (update-details
 ...
 (let (
  ...
  (map-set streams stream-id 
    (merge stream {payment-per-block: payment-per-block, timeframe: timeframe}))
    (ok true)
 )
)
;;
...
(define-read-only (hash-stream )
  ...
  (let (
    ...
    (msg (concat (concat (unwrap-panic (to-consensus-buff? stream)) 
